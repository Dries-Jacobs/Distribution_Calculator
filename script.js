(function () {
  ...
  const inputMaxDis = document.getElementById('inputMaxDis');
  const inputSpa = document.getElementById('inputSpa');
  const result1El = document.getElementById('calcDis');
  const result2El = document.getElementById('nDis');
  ...
  function calculate() {
    ...
    const b = toNumber(inputMaxDis.value);
    const c = toNumber(inputSpa.value);

    const num = Math.ceil(a / b); // number of distributions
    let calcDis = 0;
    if (num > 1) {
      calcDis = (a-(c*(num+1))) / num; // length of distributions
    }

    // Update UI
    result1El.textContent = Number.isFinite(calcDis) ? `${fmt(calcDis)} mm` : '-';
        ...
        return `
          <li class="history-item">
            ...
            <div class="row2">
              ...
              <strong>Spacing: ${fmt(h.calcDis)} mm</strong>
            </div>
          </li>`;
      })
      .join('');
  ...
  function addHistory(entry) {
    ...
    if (!Number.isFinite(entry.calcDis)) return;
